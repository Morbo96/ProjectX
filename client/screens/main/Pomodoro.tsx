...
function Pomodoro():JSX.Element{
    ...
    return (
        <ScreenView style={[pomodoroLayout.mainView]}>
            <View style={[flex.d_flex, flex.flex_row, flex.align_start, flex.justify_center]}>
                <Text style={headers.header_1}>Задача:</Text>
                <Text style={[pomodoroLayout.taskTitle,padding.pl_3, padding.pt_1,{maxWidth: "50%"}]}>Название задачи</Text>
            </View>
                    ...
                    {({ remainingTime }) => {
                        ...
                        return (
                            <View style={[flex.d_flex, flex.flex_column, flex.justify_center, flex.align_center]}>
                                <Text style={pomodoroLayout.timeText}>{minutes}:{seconds}</Text>
                                <Text style={[pomodoroLayout.hitCountText, margin.mt_3]}>Hit count: {hitCount}</Text>
                            </View>
                        )
                    }}
                </CountdownCircleTimer>
            </TouchableOpacity>
            <View style={[flex.d_flex, flex.justify_center, flex.align_center, padding.pl_3, {minHeight:"25%"}]}>
                {
                    timerEnabled?
                        <View style={[flex.d_flex, flex.flex_row, flex.justify_space_between, flex.align_center, {width:"55%"}]}>
                            <TouchableOpacity onPress={() => setKey(prevKey => prevKey + 1)}>
                                <Image source={require("../../assets/icons/restart_button.png")}/>
                            </TouchableOpacity>
                            {
                                paused?
                                    ...
                                    <TouchableOpacity onPress={()=> setPause(true)}>
                                        <Image source={require("../../assets/icons/pause_button.png")}/>
                                    </TouchableOpacity>
                        }
                            <TouchableOpacity
                                onPress={()=>{
